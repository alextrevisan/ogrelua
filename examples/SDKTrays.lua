...
function SdkTrayListener:labelHit(label) end
function SdkTrayListener:sliderMoved(slider) end
...
function Widget:cleanup()
    if self.mElement then
        self.nukeOverlayElement(self.mElement)
        self.mElement = nil
    end
end
...
-----------------------------------------------------------------------------*/
--]]
function Widget.nukeOverlayElement(element)
    local container = element
    if container:getChildIterator() then
        local toDelete = {}
        local children = container:getChildIterator()
        while children:hasMoreElements() do
            toDelete[#toDelete+1] = children:getNext()
        end
        for i = 1,#toDelete do
    ...
    if element then
        ...
        if parent then parent:removeChild(element:getName()) end
...
function Widget.isCursorOver(element, cursorPos, voidBorder)
    ...
    local l = element:_getDerivedLeft() * om:getViewportWidth()
...
function Widget.getCaptionWidth(caption, area)
    ...
    for i = 1,current.length() do
        ...
        if current[i] == ' ' then
            if area:getSpaceWidth() ~= 0 then lineWidth = lineWidth + area:getSpaceWidth()
            else lineWidth = lineWidth+ font:getGlyphAspectRatio(' ') * area:getCharHeight() end
        elseif current[i] == '\n' then break
        -- use glyph information to calculate line width
        else lineWidth = lineWidth + font:getGlyphAspectRatio(current[i]) * area:getCharHeight() end
    end
...
-----------------------------------------------------------------------------*/
--]]
function Widget.fitCaptionToArea(caption, area, maxWidth)
    local f = Ogre.FontManager:getSingleton():getByName(area:getFontName()):getPointer();
    local s = DISPLAY_STRING_TO_STRING(caption);

    local nl = s.find('\n');
    if nl ~= Ogre.String:npos() then s = s.substr(0, nl) end

    local width = 0;

    for i = 1, s.length() do
        if s[i] == ' ' and area:getSpaceWidth() ~= 0 then
            width = width + area:getSpaceWidth()
        else 
            width = width + f:getGlyphAspectRatio(s[i]) * area:getCharHeight()
...
function Widget:getOverlayElement()
    return self.mElement
end

function Widget:getName()
    return self.mElement:getName()
end

function Widget:getTrayLocation()
    return self.mTrayLoc
end
...
function Widget:isVisible()
    return self.mElement:isVisible()
end

-- callbacks

function Widget:_cursorPressed(cursorPos) end
function Widget:_cursorReleased(cursorPos) end
function Widget:_cursorMoved(cursorPos) end
function Widget:_focusLost() end

-- internal methods used by SdkTrayManager. do not call directly.

function Widget:_assignToTray(trayLoc)
    self.mTrayLoc = trayLoc
end
...
function Button.create(name, caption, width)
    ...
    if width > 0 then
        self.mElement:setWidth(width);
        self.mFitToContents = false;
    else 
        self.mFitToContents = true 
    end
